//! A runnable Rust example: back-test a buy-and-hold strategy against a thin
//! order book and print the market impact the walk measured. Every language
//! example runs the same `thin_book` request and prints the same summary — that
...
fn main() {
    let mut impact = Impact::new(SPEC).expect("valid spec");
