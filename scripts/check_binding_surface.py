...
make, so this reads `wickra_impact_<name>` out of
`bindings/c/include/wickra_impact.h` and checks each language's public surface
...
HEADER = os.path.join(ROOT, "bindings", "c", "include", "wickra_impact.h")

# Exports that are ABI plumbing rather than a promise to callers.
ABI_ONLY = {"free"}

EXPORT = re.compile(r"\bwickra_impact_([a-z0-9_]+)\s*\(")
...
# file only re-exports `Impact` from the compiled module, so the methods
...
BINDINGS: dict[str, tuple[list[str], dict[str, str]]] = {
    "python": (
        ["bindings/python/src/lib.rs", "bindings/python/python/wickra_impact/__init__.py"],
        {
            "new": r"(?m)^\s*fn new\s*\(",
    ...
    "csharp": (
        ["bindings/csharp/WickraImpact/Impact.cs"],
        {
            "new": r"(?m)^\s*public Impact\s*\(",
    ...
    "go": (
        ...
        {
            "new": r"(?m)^func New\s*\(",
            "command": r"(?m)^func \([^)]*\*Impact\) Command\s*\(",
            "version": r"(?m)^func Version\s*\(",
        },
    ),
    "java": (
        ["bindings/java/src/main/java/org/wickra/impact/Impact.java"],
        {
            "new": r"(?m)^\s*public Impact\s*\(",
            "command": r"(?m)^\s*public [^\n]*\bcommand\s*\(",
            "version": r"(?m)^\s*public static [^\n]*\bversion\s*\(",
    ...
    "cpp": (
        ["bindings/c/include/wickra_impact.hpp"],
        {
            "new": r"\bImpact\s*\(\s*(?:const\s+)?std::string",
...
def main() -> int:
    ...
    if not exports:
        print(f"no wickra_impact_* exports found in {HEADER}", file=sys.stderr)
        return 1
