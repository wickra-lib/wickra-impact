...
takes the header and library from `WKIMPACT_INC` / `WKIMPACT_LIB`, supplied from
outside, and compiles the hand-written `src/wickra_impact.c` against whatever
...
This makes the pairing explicit:
  ...
  * Every `wickra_*` symbol `src/wickra_impact.c` calls must be declared in
    `bindings/c/include/wickra_impact.h`. A call to a symbol that is not
...
downloads a prebuilt `wickra-impact-c-<triple>.tar.gz` for the version in
...
HEADER = "bindings/c/include/wickra_impact.h"
